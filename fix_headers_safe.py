...
html_files = glob.glob('views/**/*.html', recursive=True)

for file in html_files:
    if 'client' in file.replace('\\', '/') and 'admin-clients' not in file and 'manager/clients' not in file:
        continue
    ...
    if h1_start != -1:
        ...
        if 'dashboard-main-title' not in h1_tag and 'sidebar-logo' not in h1_tag:
            if 'class="' in h1_tag:
            ...
            # Update header_end since content length changed
            header_end += len(new_h1) - len(h1_tag)
            
    # 3. Fix top-header-actions
